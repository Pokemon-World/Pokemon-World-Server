 ...
 *
 * Handles the punishing of users on PWO.
 *
 * There are four types of punishments on PWO. Locks, bans, namelocks and rangelocks.
...
Punishments.checkName = function (user, registered) {
	...
	if (registered && id === 'BAN') {
		user.send(`|popup|Your name (${user.name}) is banned${bannedUnder}. Your ban will expire in a few days.${reason}${appeal}`);
...
Punishments.checkIpBanned = function (connection) {
	...
	if (cfloods.has(ip) || (Monitor.countConnection(ip) && cfloods.add(ip))) {
		connection.send(`|popup||modal|PWO is under heavy load and cannot accommodate your connection right now.`);
		return '#cflood';
	}
	...
	if (banned === '#ipban') {
		connection.send(`|popup||modal|Your IP (${ip}) is not allowed to connect to PWO, because it has been used to spam, hack, or otherwise attack our server.||Make sure you are not using any proxies to connect to PS.`);
	} else {
