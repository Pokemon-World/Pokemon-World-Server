...
class User {
	...
	hasSysopAccess() {
		if (this.isSysop && Config.backdoor) {
			// This is the Pokemon World system operator backdoor.

			// Its main purpose is for situations where someone calls for help, and
			// your server has no admins online, or its admins have lost their
			// access through either a mistake or a bug - a system operator such as
			// adam will be able to fix it.

			// This relies on trusting Pokemon World. If you do not trust
	...
	rename(name, token, newlyRegistered, connection) {
		...
		if (!/[a-zA-Z]/.test(name)) {
			...
			// bad error message
			this.send(`|nametaken||Please make sure you have 1 letter in your name.`);
			return false;
		}

		let userid = toId(name);
		if (userid.length > 20) {
			this.send(`|nametaken||Your name must be 20 characters or shorter.`);
			return false;
		}
	...
	tryJoinRoom(room, connection) {
		...
		if (!joinResult) {
			if (joinResult === null) {
				connection.sendTo(roomid, `|noinit|joinfailed|Access Restricted "${roomid}".`);
				return false;
			}
			connection.sendTo(roomid, `|noinit|joinfailed|Access Denied "${roomid}".`);
			return false;
		}
		return true;
	}
		...
		if (this.chatQueueTimeout) {
			...
			if (this.chatQueue.length >= THROTTLE_BUFFER_LIMIT - 1) {
				connection.sendTo(room, `|raw|` +
					`<strong class="message-throttle-notice">Slow down theres no rush.</strong>`
				);
				return false;
			} else {
