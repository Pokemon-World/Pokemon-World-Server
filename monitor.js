 ...
 *
 * Various utility functions to make sure PWO is running healthy.
 *
 * @license MIT license
 */
'use strict';
...
const Monitor = module.exports = {
	...
	countPrepBattle: function (ip, connection) {
		...
		if (count > 15) {
			connection.popup(`Due to high load, you are limited to 15 battles and team validations every 3 minutes.`);
			return true;
		}
	},
	/**
	...
	countConcurrentBattle: function (count, connection) {
		if (count > 8) {
			connection.popup(`Due to high load, you are limited to 8 games at the same time.`);
			return true;
		}
	},
	/**
