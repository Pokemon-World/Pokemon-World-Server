/**
 * System commands
 * Pokemon World - http://pokemon-world.online/
 *
 * These are system commands - commands required for Pokemon World
...
exports.commands = {
	...
	version: function (target, room, user) {
		...
		this.sendReplyBox("PWO Version: <b>" + Chat.package.version + "</b>");
	},
	...
	authority: function (target, room, user, connection) {
		...
		if (!buffer.length) return connection.popup("This server has 0 staff.");
	...
	avatar: function (target, room, user) {
		...
		if (!parts[1]) {
			this.sendReply("Avatar changed to:\n" +
				'|raw|<img src="//main.pokemon-world.online/sprites/trainers/' + (typeof avatar === 'string' ? avatar.substr(1) : avatar) + '.png" alt="" width="80" height="80" />');
		}
	},
	avatarhelp: ["/avatar [avatar number 1 to 293] - Choose your trainer sprite."],
	...
	invite: function (target, room, user) {
		...
		if (targetUser in targetRoom.users) return this.errorReply(`This person is already in "${targetRoom.title}".`);
	...
	makechatroom: function (target, room, user, connection, cmd) {
		...
		if (cmd === 'makeprivatechatroom') {
			...
			if (Rooms.get('upperstaff')) {
				Rooms.get('upperstaff').add('|raw|<div class="broadcast-green">Private channel was created: <b>' + Chat.escapeHTML(target) + '</b></div>').update();
			}
			this.sendReply("The private chat room '" + target + "' was created.");
		} else {
			if (Rooms.get('staff')) {
				Rooms.get('staff').add('|raw|<div class="broadcast-green">Public channel created: <b>' + Chat.escapeHTML(target) + '</b></div>').update();
			}
			if (Rooms.get('upperstaff')) {
				Rooms.get('upperstaff').add('|raw|<div class="broadcast-green">Public channel created: <b>' + Chat.escapeHTML(target) + '</b></div>').update();
			}
	...
	makegroupchat: function (target, room, user, connection, cmd) {
		...
		if (existingRoom && !existingRoom.modjoin) return this.errorReply("The channel '" + title + "' already exists.");
		...
		let groupChatURL = '';
		if (Config.serverid) {
			groupChatURL = 'http://' + (Config.serverid === 'showdown' ? 'psim.online' : Config.serverid + '.psim.online') + '/' + roomid;
	...
	deleteroom: function (target, room, user) {
		...
		if (!targetRoom) return this.errorReply("The channel '" + target + "' doesn't exist.");
		if (room.isPersonal) {
		...
		if (targetRoom.id === 'global') {
			return this.errorReply("This channel can't be deleted.");
		}

		if (targetRoom.chatRoomData) {
			if (targetRoom.isPrivate) {
				if (Rooms.get('upperstaff')) {
					Rooms.get('upperstaff').add(`|raw|<div class="broadcast-red">Private channel deleted by ${user.userid}: <b>${Chat.escapeHTML(target)}</b></div>`).update();
				}
			} else {
				if (Rooms.get('staff')) {
					Rooms.get('staff').add('|raw|<div class="broadcast-red">Public channel deleted: <b>' + Chat.escapeHTML(target) + '</b></div>').update();
				}
				if (Rooms.get('upperstaff')) {
		...
		targetRoom.add("|raw|<div class=\"broadcast-red\"><b>This channel has been deleted.</b></div>");
		targetRoom.update(); // |expire| needs to be its own message
		targetRoom.add("|expire|This channel has been deleted.");
		this.sendReply("The channel '" + target + "' was deleted.");
		targetRoom.update();
		targetRoom.destroy();
	},
	...
	privateroom: function (target, room, user, connection, cmd) {
		...
		default:
			if (room.isPrivate === true && target !== 'force') {
				return this.sendReply(`This channel is a secret chaneel. Use "/publicroom" to make it public, or "/hiddenroom force" to force it hidden.`);
			}
			setting = 'hidden';
			break;
		}
		...
		if (target === 'off' || !setting) {
			if (!room.isPrivate) {
				return this.errorReply(`This channel is already public.`);
			}
			if (room.isPersonal) return this.errorReply(`This channel can't be made public.`);
			if (room.privacySetter && user.can('nooverride', null, room) && !user.can('makeroom')) {
				if (!room.privacySetter.has(user.userid)) {
					...
					return this.errorReply(`You can't make the channel public since you didn't make it private - only ${privacySetters} can.`);
				}
				room.privacySetter.delete(user.userid);
				if (room.privacySetter.size) {
					const privacySetters = Array.from(room.privacySetter).join(', ');
					return this.sendReply(`You are no longer forcing the channel to stay private, but ${privacySetters} also need${Chat.plural(room.privacySetter, '', 's')} to use /publicroom to make the room public.`);
				}
			}
			delete room.isPrivate;
			room.privacySetter = null;
			this.addModCommand(`${user.name} made this channel public.`);
			...
			if (room.isPrivate === setting) {
				if (room.privacySetter && !room.privacySetter.has(user.userid)) {
					...
					return this.sendReply(`This chaneel is already ${settingName}, but is now forced to stay that way until you use /publicroom.`);
				}
				return this.errorReply(`This channel is already ${settingName}.`);
			}
			room.isPrivate = setting;
			this.addModCommand(`${user.name} made this channel ${settingName}.`);
			if (room.chatRoomData) {
	...
	officialroom: function (target, room, user) {
		if (!this.can('makeroom')) return;
		if (!room.chatRoomData) {
			return this.errorReply(`/officialroom - This channel can't be made official`);
		}
		if (target === 'off') {
	...
	roomdesc: function (target, room, user) {
		...
		if (normalizedTarget.includes(' 1234567890 ')) {
			return this.errorReply(`Error: Room description must not contain the word "numbers".`);
		}
	...
	roomintro: function (target, room, user, connection, cmd) {
		...
		this.sendReply("(The channel introduction has been changed to:)");
		this.sendReply('|raw|<div class="infobox infobox-limited">' + room.introMessage.replace(/\n/g, '') + '</div>');

		this.privateModCommand(`(${user.name} changed the channelintro.)`);
	...
	deleteroomintro: function (target, room, user) {
		if (!this.can('declare', null, room)) return false;
		if (!room.introMessage) return this.errorReply("This channel does not have a introduction set.");
	...
	staffintro: function (target, room, user, connection, cmd) {
		if (!target) {
			...
			if (!room.staffMessage) return this.sendReply("This channel does not have a staff introduction set.");
	...
	join: function (target, room, user, connection) {
		...
		if (target.startsWith('main.pokemon-world.online/')) target = target.slice(25);
		if (target.startsWith('psim.online/')) target = target.slice(8);
