...
class GlobalRoom {
	constructor(roomid) {
		...
		for (let i = 0; i < this.chatRoomData.length; i++) {
			...
			if (!Config.quietconsole) console.log("NEW CHANNEL: " + id);
	...
	startLockdown(err, slow) {
		...
		Rooms.rooms.forEach((curRoom, id) => {
			...
			if (!slow && curRoom.requestKickInactive && !curRoom.battle.ended) {
				...
				if (curRoom.modchat !== '+') {
					curRoom.modchat = '+';
					curRoom.addRaw(`<div class="broadcast-gold"><b>This channel +!</b><br />has been restricted only staff can talk right now.</div>`).update();
				}
			}
		});
	...
	reportCrash(err) {
		...
		const stack = (err ? Chat.escapeHTML(err.stack).split(`\n`).slice(0, 2).join(`<br />`) : ``);
		const crashMessage = `|html|<div class="broadcast-blue"><b>The server has crashed :</b> ${stack}</div>`;
